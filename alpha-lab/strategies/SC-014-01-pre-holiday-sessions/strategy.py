"""SC-014-01, the pre-holiday sessions (build-plan.md): the five equity funds in equal parts over the
scheduled session `offset` sessions before each of the exchange's scheduled holidays that falls on a
weekday, 1 the last before it, cash otherwise, on the New York Stock Exchange's calendar of
...
def pre_holiday(sessions: pd.DatetimeIndex, offset: int) -> pd.Series:
    """For each scheduled session, whether it is the `offset`-th scheduled session before one of the
    scheduled holidays that falls on a weekday, 1 the last before it."""
    holidays = ScheduledHolidays().holidays(start=sessions[0], end=sessions[-1])
    holidays = holidays[holidays.dayofweek < 5]              # a Saturday New Year's Day is none
    before = sessions.searchsorted(holidays) - offset                        # the session `offset` before
    held = np.zeros(len(sessions), dtype=bool)
    held[before[before >= 0]] = True
    return pd.Series(held, index=sessions)


def positions(market, offset):
    if offset < 1:
        raise ValueError("offset is 1 or more")
    ...
    held = pre_holiday(sessions, offset)
    # 2. on each session of the market, whether the next scheduled session is held
