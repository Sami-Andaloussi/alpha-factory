"""SC-008-01, the turn of the month (build-plan.md): the five equity funds in equal parts over the
scheduled sessions from `start` before a month's end to `end` after it, cash otherwise, on the New
York Stock Exchange's calendar of scheduled sessions derived from its holiday rules alone."""
import numpy as np
import pandas as pd
