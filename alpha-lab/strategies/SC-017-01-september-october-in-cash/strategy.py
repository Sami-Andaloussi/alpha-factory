"""SC-017-01, out of equities in September and October (build-plan.md): the five equity funds in
equal parts in every calendar month but the `months` from the month named `first`, cash in those,
on the New York Stock Exchange's calendar of scheduled sessions derived from its holiday rules
alone (SC-008-01's calendar, copied)."""
import numpy as np
import pandas as pd
...
def months_out(first: str, months: int) -> set[int]:
    """The numbers of the `months` consecutive calendar months from the month named `first`."""
    start = MONTHS.index(first) + 1
    return {(start - 1 + k) % 12 + 1 for k in range(months)}


def positions(market, first, months):
    if first not in MONTHS or not 1 <= months <= 11:
        raise ValueError("first is an English month name, months from 1 to 11")
    ...
    # 2. the months out
    out = months_out(first, months)
    # 3. on each session of the market, whether the next scheduled session falls in a month held
    after = sessions[sessions.searchsorted(index, side="right")]
    holding = pd.Series(~np.isin(after.month, sorted(out)), index=index)
