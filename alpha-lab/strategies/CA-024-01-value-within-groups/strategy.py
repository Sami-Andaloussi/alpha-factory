"""CA-024-01, value within groups (build-plan.md): in each of three groups of comparable funds, the
bottom third by the return over the value window, the last `skip` sessions left out, held in equal
shares of the group's share of the portfolio, set on the first session of each month; CA-001-01's
construction, the ranking reversed."""
import numpy as np
import pandas as pd
...
def positions(market, lookback, skip):
    ...
    # 1. the value window's return: from `lookback` to `skip` sessions before the session before
    ...
    # 3. the cheapest of each group: its bottom third of ranked members, rounded, one at least
    ...
    # 4. the weights: a group of n ranked members of N trading holds n/N among its cheapest;
    ...
    for tickers in members.values():
        ...
        cheapest = np.maximum(1, np.rint(n / 3))
        order = past[tickers].where(ranked[tickers]).rank(axis=1, ascending=True, method="first")
        chosen = order.le(cheapest, axis=0) & ranked[tickers]
        share = (n / trading.where(trading > 0) / cheapest).fillna(0.0)
