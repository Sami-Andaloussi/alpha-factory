"""SC-010-01, the first half of the trading month (build-plan.md): SC-008-01's rule with the first
half as its window, the five equity funds in equal parts over the scheduled sessions from `start`
before a month's end to `end` after it, -1 to +9, cash otherwise, on the New York Stock Exchange's
calendar of scheduled sessions derived from its holiday rules alone."""
import numpy as np
import pandas as pd
...
def in_window(sessions: pd.DatetimeIndex, start: int, end: int) -> pd.Series:
    """For each scheduled session, whether it is among the last -start sessions of its month or
    among the first `end` sessions of its month."""
    months = pd.Series(sessions.to_period("M"), index=sessions)
    forward = months.groupby(months).cumcount() + 1                           # +1, +2, ...
    backward = months.groupby(months).cumcount(ascending=False) + 1           # 1 for the last: -1
    return (backward <= -start) | (forward <= end)


def positions(market, start, end):
    if start > -1 or end < 1:
        raise ValueError("start is -1 or earlier, end +1 or later")
